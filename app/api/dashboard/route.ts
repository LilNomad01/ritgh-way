...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [academic, rows] = await Promise.all([computeAcademicState(auth.sub), db.batch([
      ...
      db.prepare("SELECT l.id, l.title, l.description FROM lessons l JOIN course_sections s ON s.id = l.section_id JOIN course_modules m ON m.id = s.module_id WHERE l.status = 'Publicado' AND s.status = 'Publicado' AND m.status = 'Publicado' ORDER BY m.position, s.position, l.position, l.id"),
    ])]);
    ...
    const lessons = rows[3].results as { id: number; title: string; description: string }[];
