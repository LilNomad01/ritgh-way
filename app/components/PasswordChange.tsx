...
export function PasswordChange({ onClose, onChanged }: { onClose: () => void; onChanged: () => void }) {
  ...
  return <div className="admin-editor-backdrop"><form className="admin-editor password-editor" onSubmit={submit}>
    <div className="editor-head"><div><span className="eyebrow">SEGURANÇA DA CONTA</span><h2>Crie sua senha definitiva</h2></div><button type="button" onClick={onClose}>×</button></div>
    <p>A senha inicial foi criada apenas para o primeiro acesso. Troque-a agora para proteger a administração.</p>
    ...
    <div className="editor-actions"><button type="button" onClick={onClose}>Agora não</button><button type="submit" disabled={saving}>{saving ? "Protegendo conta..." : "Salvar nova senha"}</button></div>
  </form></div>;
}
