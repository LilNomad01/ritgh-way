...
export function Onboarding({ onComplete, onDemo }: { onComplete: (profile: Profile) => void; onDemo: () => void }) {
  ...
  async function login() {
    ...
    setLoginError("");
    try {
      const response = await fetch("/api/account", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "login", email: loginEmail }) });
      ...
      localStorage.setItem("rightway-profile-v3", JSON.stringify(result.profile));
  ...
  async function finish() {
    ...
    try {
      const response = await fetch("/api/admin", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ entity: "student", ...profile, status: "Ativo" }) });
      if (!response.ok) throw new Error("Não foi possível criar seu perfil.");
      localStorage.setItem("rightway-profile-v3", JSON.stringify(profile));
      onComplete(profile);
    } catch {
      localStorage.setItem("rightway-profile-v3", JSON.stringify(profile));
      onComplete(profile);
    ...
    return (
      <div className="entry-shell">
        ...
        <main className="entry-main">
          ...
          <section className="entry-copy"><span className="entry-kicker">APRENDA. PRATIQUE. EVOLUA.</span><h1>Seu inglês começa<br/>do <em>jeito certo.</em></h1><p>Aulas curtas, prática todos os dias e uma jornada feita para o seu nível.</p><button className="entry-primary" onClick={() => setStep("signup")}>COMEÇAR AGORA</button><button className="entry-secondary" onClick={() => setStep("login")}>JÁ TENHO UMA CONTA</button><small>Leva menos de 3 minutos para descobrir seu nível.</small></section>
        </main>
  ...
  if (step === "login") {
    return (
      <div className="flow-shell login-shell"><header className="flow-header"><button onClick={back}>←</button><div className="entry-brand"><div className="brand-icon"><img src="/right-way-brand.png" alt="" /></div><div><strong>RIGHT WAY</strong><small>ONLINE</small></div></div><span /></header><main className="login-card"><div className="flow-mascot small"><div className="entry-eagle"><img src="/right-way-brand.png" alt="" /></div></div><span className="flow-kicker">BEM-VINDO DE VOLTA</span><h1>Continue de onde parou.</h1><p>Digite o e-mail usado no seu cadastro.</p><label>E-mail<input autoFocus type="email" value={loginEmail} onChange={(event) => setLoginEmail(event.target.value)} onKeyDown={(event) => event.key === "Enter" && login()} placeholder="voce@email.com" /></label>{loginError && <div className="login-error">{loginError}</div>}<button className="flow-primary" disabled={!loginEmail.includes("@") || saving} onClick={login}>{saving ? "ENTRANDO..." : "ENTRAR"}</button><div className="flow-divider"><span />OU<span /></div><button className="flow-outline" onClick={onDemo}>ENTRAR NA CONTA DE DEMONSTRAÇÃO</button><button className="flow-text" onClick={() => setStep("signup")}>Ainda não tenho uma conta</button></main></div>
    );
  }
  ...
  return (
    <div className="flow-shell">
      ...
      <main className="flow-main">
        {step === "signup" && <section className="flow-panel"><div className="flow-mascot"><div className="entry-eagle"><img src="/right-way-brand.png" alt="" /></div><div className="coach-bubble">Primeiro, quero conhecer você!</div></div><span className="flow-kicker">CRIE SUA CONTA</span><h1>Como podemos chamar você?</h1><p>Seu nome vai aparecer nas aulas e nos certificados.</p><div className="flow-fields"><label>Nome completo<input autoFocus value={form.fullName} onChange={(event) => setForm({ ...form, fullName: event.target.value })} placeholder="Seu nome" /></label><label>E-mail<input type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="voce@email.com" /></label></div><button className="flow-primary" disabled={!form.fullName.trim() || !form.email.includes("@")} onClick={() => setStep("goal")}>CONTINUAR</button><button className="flow-text" onClick={() => setStep("login")}>Já tenho uma conta</button></section>}
