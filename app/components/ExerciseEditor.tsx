...
const formats = {
  listening_transcription: { icon: "headphones", description: "O aluno ouve e escreve o que entendeu.", prompt: "Instrução antes do áudio", placeholder: "Escute e escreva a frase que você ouvir.", answer: "Transcrição do áudio", sample: "Nice to meet you.", help: "A voz lê esta frase. Ela também será usada como resposta correta." },
...
function Preview({ values, type }: { values: Values; type: keyof typeof formats }) {
  ...
  return <section className="exercise-preview"><div className="exercise-preview-heading"><MaterialIcon name="visibility" /><strong>Como o aluno vai ver</strong><span>Prévia · não salva respostas</span></div>
    ...
    {isListening(type) ? <ListeningAudio text={String(values.speech || values.correctAnswer || "")} /> : null}
    ...
    {confirmed ? <div role="status" className="exercise-preview-result"><strong>{result?.status === "correct" ? "Resposta correta!" : result?.status === "almost" ? "Quase!" : "Vamos revisar."}</strong><p>{String(values.explanation || "A explicação da professora aparecerá aqui.")}</p><p>Resposta esperada: {String(values.correctAnswer || "Ainda não definida")}</p></div> : null}
