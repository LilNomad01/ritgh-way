...
type Student = { id: number; fullName: string; email: string; level: string; placementScore: number; status: string; createdAt: string; hasAccount: number; mustChangePassword: number; lastLoginAt?: string };
...
type Lesson = Artwork & { id: number; sectionId: number; title: string; description: string; duration: string; lessonType: string; status: string; position: number; smartRotation: number; videoKey?: string; videoName?: string; videoSize?: number };
type Exercise = { id: number; lessonId: number; exerciseType: string; category: string; title: string; prompt: string; optionsJson?: string; correctAnswer: string; acceptedAnswersJson?: string; explanation: string; speech?: string; skillsJson?: string; rotationVariantsJson?: string; status: string; position: number };
...
export function AdminPanel() {
  ...
  const [passwordReset, setPasswordReset] = useState<{ student: Student; password: string; confirmPassword: string } | null>(null);
  ...
  async function save(event: FormEvent) {
    ...
    if (!editing) return;
    setError("");
    let values = editing.values;
    if (editing.entity === "exercise" || editing.entity === "examQuestion") {
      const type = String(values.exerciseType ?? values.questionType);
      const options = JSON.parse(String(values.optionsJson || "[]")) as string[];
      if (type === "choice" && (options.length < 2 || options.some(option => !option.trim()) || new Set(options.map(option => option.trim())).size !== options.length || !options.includes(String(values.correctAnswer)))) {
        setError("Preencha pelo menos duas alternativas diferentes e marque qual é a correta."); return;
      }
      values = { ...values, optionsJson: type === "choice" ? JSON.stringify(options) : "[]", ...(type === "listening" || type === "listening_transcription" ? { speech: String(values.correctAnswer) } : {}) };
    }
    const hasId = Boolean(editing.values.id);
    if (editing.entity === "student" && !hasId) {
      const password = String(editing.values.password ?? "");
      if (!isStrongPassword(password)) { setError("A senha inicial precisa ter 12 caracteres, com maiúscula, minúscula, número e símbolo."); return; }
      if (password !== String(editing.values.confirmPassword ?? "")) { setError("A confirmação da senha inicial não confere."); return; }
    }
    const response = await securedFetch("/api/admin", { method: hasId ? "PUT" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ entity: editing.entity, ...values }) });
    if (!response.ok) {
    ...
    setEditing(null);
    setEditorTab("geral");
    setNotice(hasId ? "Alterações salvas com sucesso." : editing.entity === "student" ? "Aluno criado com acesso liberado." : "Item criado com sucesso.");
    await load();
  }

  async function resetStudentPassword(event: FormEvent) {
    event.preventDefault();
    if (!passwordReset || passwordResetSaving) return;
    if (!isStrongPassword(passwordReset.password)) { setError("A nova senha precisa ter 12 caracteres, com maiúscula, minúscula, número e símbolo."); return; }
    if (passwordReset.password !== passwordReset.confirmPassword) { setError("A confirmação da nova senha não confere."); return; }
    setPasswordResetSaving(true);
    setError("");
    try {
      const response = await securedFetch("/api/admin", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "resetStudentPassword", id: passwordReset.student.id, password: passwordReset.password }) });
      const result = await readResponse(response);
      if (!response.ok) throw new Error(result.error ?? "Não foi possível redefinir a senha.");
      setNotice(String(result.message ?? "Senha redefinida com sucesso."));
  ...
  function newLessonValues(moduleId?: number, sectionId?: number) {
    ...
    return { title: "", description: "", sectionId: section?.id ?? 0, duration: "10 min", lessonType: "Vídeo + prática", status: "Rascunho", position: lessonCount + 1, smartRotation: 0 };
  }
  ...
  function newExerciseValues(lessonId: number) {
    ...
    return { lessonId, exerciseType: "listening_transcription", category: "Compreensão", title: "", prompt: "", optionsJson: "[]", correctAnswer: "", acceptedAnswersJson: "[]", explanation: "", speech: "", skillsJson: "[]", rotationVariantsJson: "[]", status: "Rascunho", position: count + 1 };
  }
  ...
  async function toggleSmartRotation(lesson: Lesson) {
    const next = lesson.smartRotation ? 0 : 1;
    const response = await securedFetch("/api/admin", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ entity: "lesson", ...lesson, smartRotation: next }) });
    if (!response.ok) setError((await readResponse(response)).error ?? "Não foi possível alterar a rotação inteligente.");
    else setNotice(next ? "Rotação inteligente ativada para esta prática." : "Rotação inteligente desativada.");
    await load();
  }
  ...
  return (
    <div className="admin-page page-view">
      ...
      {notice && <div className="admin-notice"><MaterialIcon name="check_circle" filled /><span>{notice}</span><button onClick={() => setNotice("")} aria-label="Fechar aviso"><MaterialIcon name="close" /></button></div>}
      {loading ? <div className="admin-loading">Carregando dados...</div> : <>
        ...
        {tab === "students" && <section className="admin-table-panel"><div className="panel-title"><div><h2>Alunos</h2><p>{data.students.length} cadastros na plataforma</p></div><button className="outline-button" onClick={() => setEditing({ entity: "student", values: { fullName: "", email: "", level: "Básico", status: "Ativo", placementScore: 0, password: "", confirmPassword: "" } })}>+ Adicionar aluno</button></div><div className="admin-table-wrap"><table><thead><tr><th>Aluno</th><th>Nível</th><th>Teste</th><th>Status</th><th>Acesso</th><th>Cadastro</th><th /></tr></thead><tbody>{data.students.map((student) => <tr key={student.id}><td><div className="student-cell"><span>{student.fullName.slice(0,2).toUpperCase()}</span><div><strong>{student.fullName}</strong><small>{student.email}</small></div></div></td><td><b className="level-tag">{student.level}</b></td><td>{student.placementScore}/8</td><td><b className={student.status === "Ativo" ? "status-live" : "status-draft"}>{student.status}</b></td><td><b className={student.hasAccount ? (student.mustChangePassword ? "access-pending" : "access-ready") : "access-missing"}>{student.hasAccount ? (student.mustChangePassword ? "Troca pendente" : "Liberado") : "Sem senha"}</b></td><td>{new Date(student.createdAt).toLocaleDateString("pt-BR")}</td><td><div className="row-actions"><button className="password" onClick={() => { setError(""); setPasswordReset({ student, password: "", confirmPassword: "" }); }}><MaterialIcon name="key" />Senha</button><button onClick={() => setEditing({ entity: "student", values: { ...student } })}>Editar</button><button className="delete" onClick={() => remove("student", student.id)}>Excluir</button></div></td></tr>)}</tbody></table>{!data.students.length && <p className="empty-state">Nenhum aluno cadastrado ainda.</p>}</div></section>}
      ...
      {practiceLessonId && (() => {
        ...
        return <div className="admin-editor-backdrop"><section className="admin-editor exercise-manager"><div className="editor-head"><div><span className="eyebrow">PRÁTICA DA AULA</span><h2>{lesson.title}</h2><p>{exercises.length} exercícios vinculados</p></div><button type="button" onClick={() => setPracticeLessonId(null)}><MaterialIcon name="close" /></button></div><div className={`smart-rotation-panel ${lesson.smartRotation ? "active" : ""}`}><div className="smart-rotation-icon"><MaterialIcon name="autorenew" filled /></div><div><strong>Rotação inteligente</strong><p>Alterna enunciados, respostas e ordem das questões a cada nova tentativa, evitando repetir a sequência anterior.</p></div><button type="button" role="switch" aria-checked={Boolean(lesson.smartRotation)} onClick={() => toggleSmartRotation(lesson)}><span />{lesson.smartRotation ? "Ativada" : "Desativada"}</button></div><div className="exercise-manager-summary"><div><MaterialIcon name="quiz" filled /><span><strong>{exercises.length}</strong><small>Questões</small></span></div><div><MaterialIcon name="publish" /><span><strong>{exercises.filter((exercise) => exercise.status === "Publicado").length}</strong><small>Publicadas</small></span></div><div><MaterialIcon name="account_tree" /><span><strong>{exercises.reduce((total, exercise) => { try { return total + JSON.parse(exercise.rotationVariantsJson || "[]").length; } catch { return total; } }, 0)}</strong><small>Variações</small></span></div><button className="primary-button icon-button" onClick={() => setEditing({ entity: "exercise", values: newExerciseValues(lesson.id) })}><MaterialIcon name="add" />Novo exercício</button></div><div className="exercise-admin-list">{exercises.map((exercise, index) => { let variants = 0; try { variants = JSON.parse(exercise.rotationVariantsJson || "[]").length; } catch { variants = 0; } return <article key={exercise.id}><span className="exercise-order">{String(index + 1).padStart(2, "0")}</span><div><small>{exercise.category} · {exercise.exerciseType}{variants ? ` · ${variants} variações` : ""}</small><strong>{exercise.title}</strong><p>{exercise.prompt}</p></div><b className={statusClass(exercise.status)}>{exercise.status}</b><div className="exercise-row-actions"><button disabled={index === 0} onClick={() => reorderExercises(exercise.id, -1)} aria-label="Mover exercício para cima"><MaterialIcon name="arrow_upward" /></button><button disabled={index === exercises.length - 1} onClick={() => reorderExercises(exercise.id, 1)} aria-label="Mover exercício para baixo"><MaterialIcon name="arrow_downward" /></button><button onClick={() => setEditing({ entity: "exercise", values: { ...exercise } })} aria-label="Editar exercício"><MaterialIcon name="edit" /></button><button className="delete" onClick={() => remove("exercise", exercise.id)} aria-label="Excluir exercício"><MaterialIcon name="delete" /></button></div></article>; })}{!exercises.length && <div className="exercise-empty"><MaterialIcon name="quiz" /><strong>Esta aula ainda não tem prática</strong><p>Crie exercícios compactos de compreensão, listening, correção ou produção.</p><button onClick={() => setEditing({ entity: "exercise", values: newExerciseValues(lesson.id) })}>Criar primeiro exercício</button></div>}</div></section></div>;
      })()}
      {examSectionId && (() => {
      ...
      {editing && <div className="admin-editor-backdrop"><form className={`admin-editor ${editing.entity === "exercise" || editing.entity === "examQuestion" ? "exercise-editor" : ""}`} onSubmit={save}>
        ...
        {editing.entity === "student" && <><label>Nome completo<input required value={String(editing.values.fullName ?? "")} onChange={(event) => setEditing({ ...editing, values: { ...editing.values, fullName: event.target.value } })} /></label><label>E-mail<input required type="email" value={String(editing.values.email ?? "")} onChange={(event) => setEditing({ ...editing, values: { ...editing.values, email: event.target.value } })} /></label><div className="form-row"><label>Nível<select value={String(editing.values.level)} onChange={(event) => setEditing({ ...editing, values: { ...editing.values, level: event.target.value } })}><option>Começando do zero</option><option>Básico</option><option>Intermediário</option><option>Avançado</option></select></label><label>Status<select value={String(editing.values.status)} onChange={(event) => setEditing({ ...editing, values: { ...editing.values, status: event.target.value } })}><option>Ativo</option><option>Pausado</option></select></label></div>{!editing.values.id && <><div className="student-password-intro"><MaterialIcon name="lock" /><div><strong>Crie o acesso do aluno</strong><span>Ele usará esta senha no primeiro login e depois criará uma senha pessoal.</span></div></div><div className="form-row"><label>Senha inicial<input required minLength={12} type="password" autoComplete="new-password" value={String(editing.values.password ?? "")} onChange={(event) => setEditing({ ...editing, values: { ...editing.values, password: event.target.value } })} /></label><label>Confirmar senha<input required minLength={12} type="password" autoComplete="new-password" value={String(editing.values.confirmPassword ?? "")} onChange={(event) => setEditing({ ...editing, values: { ...editing.values, confirmPassword: event.target.value } })} /></label></div><small className="password-requirements">12+ caracteres · maiúscula · minúscula · número · símbolo</small></>}</>}
      ...
      {passwordReset && <div className="admin-editor-backdrop"><form className="admin-editor password-editor student-password-reset" onSubmit={resetStudentPassword}>
        <div className="editor-head"><div><span className="eyebrow">ACESSO DO ALUNO</span><h2>Redefinir senha</h2></div><button type="button" onClick={() => setPasswordReset(null)}><MaterialIcon name="close" /></button></div>
        <div className="password-student-card"><span>{passwordReset.student.fullName.slice(0, 2).toUpperCase()}</span><div><strong>{passwordReset.student.fullName}</strong><small>{passwordReset.student.email}</small></div></div>
        <p>A senha anterior será substituída e todas as sessões abertas serão encerradas. No próximo acesso, o aluno deverá criar uma senha pessoal.</p>
        {error && <div className="admin-alert">{error}</div>}
        <label>Nova senha inicial<input autoFocus required minLength={12} type="password" autoComplete="new-password" value={passwordReset.password} onChange={(event) => setPasswordReset({ ...passwordReset, password: event.target.value })} /></label>
        <label>Confirmar nova senha<input required minLength={12} type="password" autoComplete="new-password" value={passwordReset.confirmPassword} onChange={(event) => setPasswordReset({ ...passwordReset, confirmPassword: event.target.value })} /></label>
        <small>Use 12 ou mais caracteres com letra maiúscula, minúscula, número e símbolo. A senha nunca será exibida novamente.</small>
        <div className="editor-actions"><button type="button" onClick={() => setPasswordReset(null)}>Cancelar</button><button type="submit" disabled={passwordResetSaving}>{passwordResetSaving ? "Salvando..." : "Redefinir senha"}</button></div>
      </form></div>}
    </div>
  );
}
