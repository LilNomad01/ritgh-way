...
type Data = { students: Student[]; modules: Module[]; sections: Section[]; lessons: Lesson[]; exercises: Exercise[] };
type Entity = "student" | "module" | "section" | "lesson" | "exercise";

const emptyData: Data = { students: [], modules: [], sections: [], lessons: [], exercises: [] };
...
export function AdminPanel() {
  ...
  return (
    <div className="admin-page page-view">
      ...
      {editing && <div className="admin-editor-backdrop"><form className={`admin-editor ${editing.entity === "exercise" ? "exercise-editor" : ""}`} onSubmit={save}>
        <div className="editor-head"><div><span className="eyebrow">{editing.values.id ? "EDITAR" : "NOVO ITEM"}</span><h2>{editing.entity === "student" ? "Aluno" : editing.entity === "module" ? "Módulo" : editing.entity === "section" ? "Seção" : editing.entity === "exercise" ? "Exercício" : "Aula"}</h2></div><button type="button" onClick={() => setEditing(null)}><MaterialIcon name="close" /></button></div>
        ...
        {editing.entity === "section" && <><label>Título<input required value={String(editing.values.title ?? "")} onChange={(event) => setEditing({ ...editing, values: { ...editing.values, title: event.target.value } })} /></label><label>Módulo<select value={String(editing.values.moduleId)} onChange={(event) => setEditing({ ...editing, values: { ...editing.values, moduleId: Number(event.target.value) } })}>{data.modules.map((module) => <option value={module.id} key={module.id}>{module.title}</option>)}</select></label></>}
        {editing.entity === "lesson" && <><label>Título<input required value={String(editing.values.title ?? "")} onChange={(event) => setEditing({ ...editing, values: { ...editing.values, title: event.target.value } })} /></label><label>Seção<select value={String(editing.values.sectionId)} onChange={(event) => setEditing({ ...editing, values: { ...editing.values, sectionId: Number(event.target.value) } })}>{data.sections.map((section) => <option value={section.id} key={section.id}>{section.title}</option>)}</select></label><div className="form-row"><label>Duração<input value={String(editing.values.duration ?? "")} onChange={(event) => setEditing({ ...editing, values: { ...editing.values, duration: event.target.value } })} /></label><label>Status<select value={String(editing.values.status)} onChange={(event) => setEditing({ ...editing, values: { ...editing.values, status: event.target.value } })}><option>Rascunho</option><option>Publicado</option></select></label></div><label>Tipo de aula<input value={String(editing.values.lessonType ?? "")} onChange={(event) => setEditing({ ...editing, values: { ...editing.values, lessonType: event.target.value } })} /></label>{editing.values.id && <div className="lesson-practice-link"><MaterialIcon name="quiz" filled /><div><span>PRÁTICA DA AULA</span><strong>Exercícios vinculados: {data.exercises.filter((exercise) => exercise.lessonId === Number(editing.values.id)).length}</strong><small>A prática só aparece para o aluno quando houver questões publicadas.</small></div><button type="button" onClick={() => { setPracticeLessonId(Number(editing.values.id)); setEditing(null); }}>Gerenciar exercícios</button></div>}</>}
        ...
        <div className="editor-actions"><button type="button" onClick={() => setEditing(null)}>Cancelar</button><button type="submit">Salvar alterações</button></div>
      </form></div>}
    </div>
  );
}
