...
type LessonDetailPayload = { videos: LessonVideo[]; lesson: CourseLesson & { sectionTitle: string; sectionDescription: string; moduleTitle: string; moduleId: number; level: string; videoName?: string }; state: LessonState };
...
function LessonPage({ lessonId, onBack, onPractice }: { lessonId: number; onBack: () => void; onPractice: (lessonId: number) => void }) {
  ...
  async function saveVideoProgress(ended = false) {
    ...
    if (response.ok) {
      ...
      if (data.state) setPayload((current) => current ? { ...current, state: data.state as LessonState, videos: current.videos.map(item => item.id === selectedVideo?.id ? { ...item, savedPosition, completed: ended ? 1 : item.completed, updatedAt: new Date().toISOString() } : item) } : current);
